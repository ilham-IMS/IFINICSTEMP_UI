...
{
  public partial class IncentiveSchemeForm
  {
    #region Service
    ...
    {
      Loading.Show();
      var res = await IFINICSClient.GetRow<JsonObject>("IncentiveScheme", "GetRowByID", new
      {
        ID = ID
      });
    ...
    private async Task OnSubmit(JsonObject data)
    {
      ...
      if (ID == null)
      {
        var res = await IFINICSClient.Post("IncentiveScheme", "Insert", data);

        if (res?.Data != null)
        {
          NavigationManager.NavigateTo($"{ParentMenuURL}/incentivescheme/{res.Data["ID"]}");
        }
      }
      #endregion
    ...
    private void Back()
    {
      NavigationManager.NavigateTo($"{ParentMenuURL}/incentivescheme");
    }
    #endregion
    ...
    private async Task ChangeActive()
    {
      if (ID != null)
      {
        ...
        var res = await IFINICSClient.Put("IncentiveScheme", "ChangeStatus", data);
