...
{
  public partial class IncentiveMarketingDataGrid
  {
    #region Service
    ...
    {
      BodyResponse<List<JsonObject>>? res = await IFINICSClient.GetRows<JsonObject>("IncentiveMarketing", "GetRows", new
      {
        args.Keyword,
        args.Offset,
        args.Limit,
    ...
    private string GetLink(JsonObject row)
    {
      ...
      return $"incentivecalculation/marketing/{row["ID"]}";
